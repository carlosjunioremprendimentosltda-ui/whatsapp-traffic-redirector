...
function injectHiddenId(message = '', id) {
  ...
  // Injeta NO MEIO da frase (estilo Tintim) para proteger contra o trim() e sanitização do WhatsApp no final do texto!
  const words = message.trim().split(/\s+/);
  if (words.length >= 4) {
    const splitIndex = Math.max(2, Math.floor(words.length * 0.6));
    const firstPart  = words.slice(0, splitIndex).join(' ');
    const secondPart = words.slice(splitIndex).join(' ');
    return firstPart + invisiblePayload + ' ' + secondPart;
  }

  // Se tiver pontuação (vírgula, exclamação ou ponto), injeta antes do resto
  if (message.includes(',')) {
    const parts = message.split(',');
    return parts[0] + ',' + invisiblePayload + parts.slice(1).join(',');
  }
